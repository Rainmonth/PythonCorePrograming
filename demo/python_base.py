...
help(obj.union('s'))  # 查询obj.func的具体介绍和用法
...
# int(string, base)
int('123', base=16)  # 将字符串转化为整数，base为进制数
...
y = Fraction("0.25")  # 分数类型 1/4 接收字符串类型的参数
...
L = M = [1, 2]
L = L + [3, 4]  # L = [1, 2, 3, 4], M = [1, 2] # 普通赋值
L += [3, 4]  # L = [1, 2, 3, 4], M = [1, 2, 3, 4] # 增强赋值
...
s1 = 's1'
s2 = 's2'
s1 + s2, s1 * 3, S[1], S[1:2], len(S)  # 字符串操作
...
len(L)  # 求列表长度
L.count(2)  # 求列表中某个值的个数
L.append(obj)  # 向列表的尾部添加数据，比如append(2)，添加元素2
L.insert(0, obj)  # 向列表的指定index位置添加数据，index及其之后的数据后移
L.extend([2, 3, 4])  # 通过添加iterable中的元素来扩展列表，比如extend([2])，添加元素2，注意和append的区别
L.index(3, 0, len(L) - 1)  # 返回列表中值value的第一个索引
L.pop([0])  # 删除并返回index处的元素，默认为删除并返回最后一个元素
L.remove(3)  # 删除列表中的value值，只删除第一次出现的value的值
L.reverse()  # 反转列表
...
D.items()  # 字典键、值以及键值对
D.get('spam', 0)  # get函数
D.update({'randy': 3, 'ham': 10})  # 合并字典，如果存在相同的键值，D_other的数据会覆盖掉D的数据
D.pop('spam', -1)  # 删除字典中键值为key的项，返回键值为key的值，如果不存在，返回默认值D，否则异常
D.popitem()  # pop字典中随机的一项（一个键值对）
D.setdefault('tol', {'test': 3})  # 设置D中某一项的默认值。如果k存在，则返回D[k]，否则设置D[k]=d，同时返回D[k]。
del D  # 删除字典
del D['key']  # 删除字典的某一项
if 'key' in D:
    print('key in D')
...
# -- 文件基本操作
fo = open(r'C:\spam', 'w')  # 打开输出文件，用于写
fp = open('data', 'r')  # 打开输入文件，用于读。打开的方式可以为'w', 'r', 'a', 'wb', 'rb', 'ab'等
size = 1024
...
fp.seek(2, 0)  # 将文件打开操作标记移到offset的位置。whence为0表示从头开始计算，1表示以当前位置为原点计算。2表示以文件末尾为原点进行计算。
...
# -- import语句from语句的as扩展
import numpy as np
from matplotlib import matplotlib_fname as name
